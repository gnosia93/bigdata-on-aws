![logo](https://github.com/gnosia93/bigdata-on-aws/blob/main/workshop/images/spark-logo.png)


Apache Spark는 하둡 MapReduce 패러다임의 한계를 극복하고자 한 등장한 범용 목적의 분산 클러스터 시스템이자 개발 프레임워크로, 연산의 중간 결과를 DISK 에 저장하는 하둡과 달리, 메모리상에 저장함으로써 하둡 대비 최대 100배의 성능을 제공합니다. Apache Spark는 오픈소스이며, 연산에 대한 Fault Tolerance 기능을 제공하고 있고, RDD, 데이터프레임 및 데이터셋 API를 이용하여 빅데이터 분석/처리 및 머신러닝 워크로드를 병렬로 처리할 수 있는 프로그래밍 인터페이스를 제공합니다. 엔진은 스칼러로 구현되어 있으나, 스칼러 뿐만 아니라 R, 자바, 파이썬으로 구현된 코드를 실행할 수 있고, DAG 기반의 연산 최적화 기능을 제공하고 있으며, 배치처리 뿐만 아니라 실시간 스트리밍 처리 기능을 제공하고 있습니다.

이번 챕터에서는 스파크 배치 및 스트리밍 API 를 이용하여, 카프카로 전송된 EC2 의 CPU 정보를 실시간으로 읽고, 처리해 보도록 하겠습니다. (데이터 프레임 API 에 대해서만 다룹니다)

### 1. 스파크 쉘 실행하기 ###

emr 마스터 노드로 로그인 한 후, package 옵션으로 kafka 패키지를 설정하여 스파크쉘을 실행합니다. 실행시 스파크쉘은 카프카 패키지를 자동으로 다운로드 받습니다.
executor-memory 파라미터 값은 1G 로 설정하여 실행합니다.(파리미터를 설정하지 않으면 JVM 메모리 오류가 발생함)
```
$ terraform output 
Outputs:
...
scala> val kdf = spark.read.format("kafka") 
                     .option("kafka.bootstrap.servers", "b-1.bigdata-msk.4hz3qf.c2.kafka.ap-northeast-2.amazonaws.com:9092,b-2.bigdata-msk.4hz3qf.c2.kafka.ap-northeast-2.amazonaws.com:9092,b-3.bigdata-msk.4hz3qf.c2.kafka.ap-northeast-2.amazonaws.com:9092")     // 브로커를 여려분들의 카프카 브로커 주소로 변경하세요.
...
var kdf = spark.readStream.format("kafka")
	       .option("kafka.bootstrap.servers", "b-1.bigdata-msk.4hz3qf.c2.kafka.ap-northeast-2.amazonaws.com:9092,b-2.bigdata-msk.4hz3qf.c2.kafka.ap-northeast-2.amazonaws.com:9092,b-3.bigdata-msk.4hz3qf.c2.kafka.ap-northeast-2.amazonaws.com:9092") 
               .option("subscribe", "cpu-metric") 
               .option("startingOffsets", "earliest") 
...
+----+--------------------+----------+---------+------+-------------------+-------------+
```
스트림 읽기를 중단하고자 하는 경우 Ctrl+C 를 누른다. 
...
```
[hadoop@ip-10-1-1-125 ~]$ spark-shell --master yarn --executor-memory 1G \
--packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.1.1
