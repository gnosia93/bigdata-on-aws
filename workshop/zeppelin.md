...
데이터 프레임은 스파크가 제공하는 구조적인 API, 칼럼과 로우를 가진 DB 테이블의 처럼 데이터를 조작하는 것을 가능하게 하는 스파크 객체로서, 데이터에 대한 조회, 필터링 및 집계 기능을 제공합니다. 아래 spark 데이터 프레임 샘플 코드를 제플린 노트북에서 실행하고, 결과를 확인 합니다. 
