...
아래 spark 데이터 프레임 코드를 제플린 노트북에서 실행하고, 결과를 하도록 합니다. 
데이터 프레임은 RDBMS 의 테이블과 같이 row 와 column 을 가지고 있는 spark API 객체로서, 데이터 프레임을 이용하면 데이터에 대한 조회, 처리 및 집계을 하실 수 있습니다.  
